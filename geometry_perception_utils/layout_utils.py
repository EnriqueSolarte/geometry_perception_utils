...
def ly2depth_from_list_corners(list_corners, camera_height, ceiling_height, shape=(512, 1024)):
    ceiling_cor = [p + np.array((0, -ceiling_height, 0)).reshape(3, 1)
                   for p in list_corners]
    floor_cor = [p + np.array((0, camera_height, 0)).reshape(3, 1)
                 for p in list_corners]

    corners = []
    [(corners.append(ceiling_cor[i]), corners.append(floor_cor[i]))
     for i in range(ceiling_cor.__len__())]
