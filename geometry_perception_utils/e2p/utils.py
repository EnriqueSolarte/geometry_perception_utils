...
__all__ = ['XY2lonlat', 'lonlat2xyz', 'XY2xyz', 'xyz2lonlat',
           'lonlat2XY', 'xyz2XY', 'EquirectTransformer']
...
def XY2lonlat(xy, shape, mode='numpy'):
    ...
    lat = lat[..., None]
    out = np.concatenate(
        [lon, lat], axis=-1) if mode == 'numpy' else torch.cat([lon, lat], dim=-1)
...
def lonlat2xyz(lonlat, RADIUS=1.0, mode='numpy'):
    ...
    out = np.concatenate(
        lst, axis=-1) if mode == 'numpy' else torch.cat(lst, dim=-1)
    return out
...
def create_grid(shape):
    ...
    mean_lonlat[:, 1] = 0
    mean_lonlat[:, 0] = ((np.arange(l) / float(l-1)) *
