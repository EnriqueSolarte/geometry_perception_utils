...
def e2p(e_img, fov_deg, u_deg, v_deg, out_hw, in_rot_deg=0, mode="bilinear"):
    ...
    hw = (out_hw[0] * _fov[0]//2).astype(np.int32), (out_hw[1]
                                                     * _fov[1]//2).astype(np.int32)
...
def e2p_boundary(e_boundary_map, fov_deg, u_deg, v_deg, out_hw, in_rot_deg=0, mode="bilinear"):
    assert len(e_boundary_map.shape) == 2
    h, w = e_boundary_map.shape[:2]
    ...
    uv = xyz2uv(xyz)
    coor_xy = uv2coor(uv, h, w)
    pers_boundary_map = sample_equirec(e_boundary_map, coor_xy, order=order)

    pers_boundary = np.zeros((pers_boundary_map.shape[1]))
    for i in range(len(pers_boundary)):
        index = np.where(pers_boundary_map[:, i] > 0)[0]
        if len(index) == 0:
            pers_boundary[i] = -1
        else:
            index = index[-1]
